...
async function fetchPosts() {
  loading.value = true
  try {
    const res = await postApi.list({ type: 'galgame', page: page.value, pageSize })
...
async function fetchRandomPosts() {
  ...
  try {
    const res = await postApi.list({ type: 'galgame', pageSize: 5, random: 'true' })
...
function linkOf(post) {
  return `/${typeRoute[post.type] || 'galgame'}/${post.id}`
}
...
<template>
  <div class="page">
    <div class="page-header">
      <h1 class="page-title">◈ Galgame 感想</h1>
      <p class="page-desc">视觉小说的感动与回味</p>
    </div>
